...
class TeacherStatusRepository {
  ...
  const TeacherStatusRepository({required this.api});
  ...
  Future<TeacherStatusList> getTeacherStatuses({
    ...
    final request = StatusQueryRequest<TeacherRosterStatus>(
      academyId: academyId,
      classId: classId,
    ...
    final response = await api.getTeacherStatuses(request);

    return TeacherStatusList(
      items: response.items
          .map(
            (item) => StatusView<Teacher, TeacherRosterStatus>(
