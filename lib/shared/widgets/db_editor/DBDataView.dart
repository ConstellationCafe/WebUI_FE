...
class _DBDataState extends State<DBDataView> {
   ...
   */
  @override
  void didUpdateWidget(covariant DBDataView oldWidget) {
  ...
  TableRow createRow(int rowIndex, DBController controller) {
    final List<int> selectedCell = controller.getSelectedCell();

    final visibleColumnIndexes =
        List.generate(controller.model.columns.length, (index) => index)
            .where(
    ...
    return TableRow(
      children: visibleColumnIndexes.map((colIndex) {
        ...
        final columnName = controller.model.columns[colIndex].toString();
        final isEditable = !widget.readOnlyColumns.contains(columnName);
        ...
        return SingleChildScrollView(
          ...
          child: Table(
            border: const TableBorder(
              top: BorderSide(color: DBEditorColors.border, width: 1),
              bottom: BorderSide(color: DBEditorColors.border, width: 1),
              left: BorderSide(color: DBEditorColors.border, width: 1),
              right: BorderSide(color: DBEditorColors.border, width: 1),
